...
export function registerTaskTools(server, client) {
    ...
    server.tool("taiga_tasks_create", "Create a new task.", {
    ...
    server.tool("taiga_tasks_get", "Get a task by ID.", { id: z.number().describe("Task ID") }, async ({ id }) => {
    ...
    server.tool("taiga_tasks_get_by_ref", "Get a task by ref and project.", {
        ref: z.number().describe("Task ref number"),
        project: z.number().describe("Project ID"),
    }, async (params) => {
        const data = await client.get("/tasks/by_ref", params);
        return {
            content: [
                { type: "text", text: JSON.stringify(data, null, 2) },
            ],
    ...
    server.tool("taiga_tasks_update", "Update a task (partial).", {
    ...
    server.tool("taiga_tasks_delete", "Delete a task.", { id: z.number().describe("Task ID") }, async ({ id }) => {
        await client.delete(`/tasks/${id}`);
        return { content: [{ type: "text", text: "Task deleted." }] };
    });
    server.tool("taiga_tasks_bulk_create", "Bulk create tasks.", {
    ...
    server.tool("taiga_tasks_filters_data", "Get filters data for tasks in a project.", { project: z.number().describe("Project ID") }, async (params) => {
        const data = await client.get("/tasks/filters_data", params);
        return {
            content: [
                { type: "text", text: JSON.stringify(data, null, 2) },
            ],
    ...
    // Voting
    server.tool("taiga_tasks_upvote", "Upvote a task.", { id: z.number().describe("Task ID") }, async ({ id }) => {
        await client.post(`/tasks/${id}/upvote`);
        return { content: [{ type: "text", text: "Task upvoted." }] };
    });
    server.tool("taiga_tasks_downvote", "Remove vote from a task.", { id: z.number().describe("Task ID") }, async ({ id }) => {
        await client.post(`/tasks/${id}/downvote`);
        return { content: [{ type: "text", text: "Vote removed." }] };
    });
    server.tool("taiga_tasks_voters", "List voters of a task.", { id: z.number().describe("Task ID") }, async ({ id }) => {
    ...
    // Watching
    server.tool("taiga_tasks_watch", "Watch a task.", { id: z.number().describe("Task ID") }, async ({ id }) => {
        await client.post(`/tasks/${id}/watch`);
        return {
            content: [{ type: "text", text: "Now watching task." }],
        };
    });
    server.tool("taiga_tasks_unwatch", "Stop watching a task.", { id: z.number().describe("Task ID") }, async ({ id }) => {
        await client.post(`/tasks/${id}/unwatch`);
        return {
            content: [{ type: "text", text: "Stopped watching task." }],
        };
    });
    server.tool("taiga_tasks_watchers", "List watchers of a task.", { id: z.number().describe("Task ID") }, async ({ id }) => {
    ...
    // Attachments
    server.tool("taiga_tasks_attachments_list", "List attachments of a task.", {
        project: z.number().describe("Project ID"),
        object_id: z.number().describe("Task ID"),
    }, async (params) => {
        const data = await client.get("/tasks/attachments", params);
        return {
            content: [
                { type: "text", text: JSON.stringify(data, null, 2) },
            ],
    ...
    server.tool("taiga_tasks_attachment_get", "Get a specific task attachment.", { id: z.number().describe("Attachment ID") }, async ({ id }) => {
