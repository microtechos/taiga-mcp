import { z } from "zod";
export function registerMembershipTools(server, client) {
    server.tool("taiga_memberships_list", "List memberships. Filter by project or role.", {
    ...
    server.tool("taiga_memberships_create", "Create a membership (invite user to project).", {
    ...
    server.tool("taiga_memberships_get", "Get a membership by ID.", { id: z.number().describe("Membership ID") }, async ({ id }) => {
    ...
    server.tool("taiga_memberships_update", "Update a membership (change role).", {
    ...
    server.tool("taiga_memberships_delete", "Delete a membership (remove user from project).", { id: z.number().describe("Membership ID") }, async ({ id }) => {
        await client.delete(`/memberships/${id}`);
        return {
            content: [{ type: "text", text: "Membership deleted." }],
        };
    });
    server.tool("taiga_memberships_bulk_create", "Bulk invite members to a project.", {
