export class TaigaClient {
    ...
    async getBinary(urlOrPath) {
        ...
        if (target.origin !== apiOrigin) {
            throw new Error(`Refusing to fetch an attachment from a non-Taiga origin (${target.origin}); only ${apiOrigin} is allowed.`);
        }
        let current = target;
        ...
        for (let hop = 0; hop < 6; hop++) {
            ...
            if (!res.ok) {
                ...
                throw new Error(`Taiga binary fetch failed ${res.status} GET ${current.origin}${current.pathname}: ${text}`);
            }
